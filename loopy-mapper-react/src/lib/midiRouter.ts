...
export class MidiRouter {
    ...
    private executeTrackAction(
        ...
        switch (soundSource.type) {
            ...
            case "liveMidi": {
                ...
                if (eventType === "noteOn") {
                    synthEngine.noteOn(voiceId, note, velocity);
                    // Record MIDI if enabled
                    if (soundSource.recordMidi && soundSource.recordedSequence) {
                        soundSource.recordedSequence.push({
                            deltaTime: 0,
                            type: "noteOn",
                            note,
    ...
    private captureLearnBinding(channel: number, triggerType: "noteOn" | "cc", value: number): void {
        ...
        // Add to store
        store.song.midiBindings.push(binding);
    ...
    autoGenerateBindings(): void {
        ...
        store.song.midiBindings = [...manualBindings, ...newBindings];
        this.rebuildBindings();
