#!/usr/bin/env bash
# ================================================================
#  Loopy Mapper — macOS / Linux Launch Script
#  Usage: ./launch.sh            Launch both frontend + backend
...
cleanup() {
  ...
  for pid in "${PIDS[@]:-}"; do
    if kill -0 "$pid" 2>/dev/null; then
      # Kill the process group so children (uvicorn workers) also die
...
if [[ "${1:-}" == "help" ]]; then
  ...
  echo "  ./launch.sh            Launch both frontend + backend"
  echo "  ./launch.sh frontend   Launch frontend only (port $FRONTEND_PORT)"
  echo "  ./launch.sh backend    Launch backend only (port $BACKEND_PORT)"
